...
export const applyjob=async(req,res)=>{
    try{
        ...
        if(existingApplication){
            return res.status(400).json({message:"you have already applying for job",success:false});
        }
...
export const getAppliedJob = async (req,res)=>{
    try{
        const userId = req.id;
        const application =(await Application.find({applicant:userId })).sort({createdAt:-1}).populate({path:"job",options:{sort:{created:-1}},
...
export const updatestatus =async (req,res)=>{
    try{
      ...
      return res.status(200).json({
        message:"application status required",success:true
      });
    }catch(error){
       console.error(error);
