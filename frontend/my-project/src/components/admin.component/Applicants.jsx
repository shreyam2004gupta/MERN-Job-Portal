...
const Applicants = () => {
    ...
    useEffect(()=>{
        const fetchAllApplicants = async ()=>{
            try{
                const res = await axios.get(`/${APPLICANTS_API_ENDPOINT}/${params._id}/applicants`,{
  ...
  return (
    <div>
      ...
      <div className='max-w-7xl mx-auto'>
        <h1 className='font-bold text-xl my-10'>Applicants {{applicants?.applicants?.length}}</h1>
        <ApplicantsTable/>
      </div>
    </div>
  )
